...
var hasNodeRequire =
	typeof require === 'function' && typeof window === 'undefined';
	...
	suite(engineName, function() {
		...
		suite('should be sliced', function() {
			...
			test('with only start offset argument given', function() {
				...
				compareBytes(pointerCopy.getBytes(), [
					0xfe,
					0xfd,
					0xfc,
					0xfa,
		...
		test('Uint64 should have precise toString', function() {
			...
			assert.equal(
				jDataView.Uint64.fromNumber(12345678901).toString(),
				'12345678901'
			);
			// Number.MAX_SAFE_INTEGER
			assert.equal(
				jDataView.Uint64.fromNumber(9007199254740992).toString(),
				'9007199254740992'
			);
			// this is larger than 2^53
			assert.equal(
				new jDataView.Uint64(12345678, 12345678).toString(),
				'53024283269292366'
			);
			// 2^64 - 1
			assert.equal(
				new jDataView.Uint64(
					Math.pow(2, 32) - 1,
					Math.pow(2, 32) - 1
				).toString(),
		...
		test('Int64 should have precise toString', function() {
			...
			// -Number.MAX_SAFE_INTEGER
			assert.equal(
				jDataView.Int64.fromNumber(-9007199254740992).toString(),
				'-9007199254740992'
			);
			...
			var toStr = function(lo, hi) {
				return new jDataView.Int64(lo, hi).toString();
			};
